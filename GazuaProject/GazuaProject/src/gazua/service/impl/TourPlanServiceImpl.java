...
public class TourPlanServiceImpl implements TourPlanService {
   ...
   public void deleteTourPlan(TourPlan tourplan) throws Exception {
      try{
         int result = sqlSession.delete("TourPlanMapper.deleteTourPlan", tourplan);
         if(result == 0){
            throw new NullPointerException();
         }
      }catch (NullPointerException e) {
   ...
   public List<TourPlan> selectTourPlanByMemberId(TourPlan tourplan) throws Exception {
      List<TourPlan> result = null;
      try {
         result = sqlSession.selectList("TourPlanMapper.selectTourPlanByMemberId", tourplan);
         
      } catch (Exception e) {
         logger.error(e.getLocalizedMessage());
         throw new Exception("회원의 일정 목록 조회에 실패했습니다.");
      }
      return result;
   }
   ...
   public void deleteTourPlanByMemberId(TourPlan tourplan) throws Exception {
      try{
         int result = sqlSession.delete("TourPlanMapper.deleteTourPlanByMemberId", tourplan);
      } catch (Exception e) {
         sqlSession.rollback();
         logger.error(e.getLocalizedMessage());
         throw new Exception("회원의 게시물 삭제에 실패했습니다.");
      }finally{
         sqlSession.commit();
      }
   }
