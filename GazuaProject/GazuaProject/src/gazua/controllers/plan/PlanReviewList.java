...
public class PlanReviewList extends BaseController {
	...
	public String doRun(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		tourplan.setName(keyword);
		tourplan.setIntro(keyword);
		tourplan.setTheme(people_level);
		
		logger.debug("people_level="+tourplan.getTheme());
		...
		if(arrange == 1){
			try {
				// 전체 게시물 수
				totalCount = tourPlanService.selectTourPlanCount(tourplan);
				...
				tourplan.setLimitStart(pageHelper.getLimitStart());
				tourplan.setListCount(pageHelper.getListCount());
				
				tourPlanList = tourPlanService.selectTourPlanListByPopular(tourplan);
				// 조회결과가 존재할 경우 --> 갤러리라면 이미지 경로를 썸네일로 교체
				for (int i=0; i<tourPlanList.size(); i++) {
					TourPlan item = tourPlanList.get(i);
			...
			try {
				...
				tourPlanList = tourPlanService.selectTourPlanListByTime(tourplan);
				// 조회결과가 존재할 경우 --> 갤러리라면 이미지 경로를 썸네일로 교체
				for (int i=0; i<tourPlanList.size(); i++) {
					TourPlan item = tourPlanList.get(i);
					photo.setTourPlan_id(item.getId());
		...
		/** (7) 조회 결과를 View에 전달 */
		request.setAttribute("tourPlanList", tourPlanList);
