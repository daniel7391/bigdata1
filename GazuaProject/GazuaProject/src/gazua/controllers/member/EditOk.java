...
public class EditOk extends BaseController {
	...
	// --> import gazua.helper.UploadHelper;
	UploadHelper upload;
	// --> import gazua.mysite.service.MemberService;
	...
	public String doRun(HttpServletRequest request, HttpServletResponse response) 
		...
		// --> import gazua.mysite.service.impl.MemberServiceImpl;
		...
		String new_member_pw = paramMap.get("new_member_pw");
		String new_member_pw_re = paramMap.get("new_member_pw_re");
		String member_email = paramMap.get("member_email");
		String member_gender = paramMap.get("gender");
		...
		logger.debug("new_member_pw=" + new_member_pw);
		logger.debug("new_member_pw_re=" + new_member_pw_re);
		...
		if (regex.isValue(new_member_pw)) {
			if (!regex.isEngNum(new_member_pw) || new_member_pw.length() > 20) {
				sqlSession.close();
			...
			// 비밀번호 확인
			if (!new_member_pw.equals(new_member_pw_re)) {
				sqlSession.close();
		...
		// 파일경로를 로그로 기록
		logger.debug("member_profile_img=" + member_profile_img);
